...
def parse_args() -> argparse.Namespace:
    ...
    data_root = Path("/home/mila/e/echchabo/scratch/SDG6-Tracker")
    ...
    parser.add_argument(
        "--pw-csv",
        type=Path,
        default=data_root / "inference" / "Nigeria" / "pw_predictions.csv",
        help="Piped water inference CSV.",
    )
    parser.add_argument(
        "--sw-csv",
        type=Path,
        default=data_root / "inference" / "Nigeria" / "sw_predictions.csv",
        help="Sewage inference CSV.",
    )
    parser.add_argument(
        "--tiles-csv",
        type=Path,
        default=data_root / "meta_pop_data" / "countries_2x2" / "Nigeria" / "nga_general_2020_tiles.csv",
        help="Tile population CSV for the target country.",
    )
    parser.add_argument(
        "--country-shapefile",
        type=Path,
        default=data_root / "meta_pop_data" / "natural_earth_data" / "ne_110m_admin_0_countries.shp",
    ...
    parser.add_argument(
        "--output-figure",
        type=Path,
        default=repo_root / "notebooks" / "nigeria_pw_sw_hotspots.png",
        help="Output figure path for the combined side-by-side PW/SW map (PNG).",
    )
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=repo_root / "notebooks" / "nigeria_access_hotspots.csv",
