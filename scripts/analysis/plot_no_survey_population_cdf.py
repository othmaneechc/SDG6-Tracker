...
def main() -> None:
    ...
    parser.add_argument(
        "--summary-csv",
        type=Path,
        default=repo_root / "data" / "inference_summary.csv",
    )
    parser.add_argument(
        "--r7-csv",
        type=Path,
        default=data_root / "R7.csv",
    )
    parser.add_argument(
        "--r8-csv",
        type=Path,
        default=data_root / "R8.csv",
    )
    parser.add_argument(
        "--r9-csv",
        type=Path,
        default=data_root / "R9.csv",
    )
    parser.add_argument(
        "--output-figure",
        type=Path,
        default=repo_root / "outputs" / "figures" / "no_survey_population_coverage_cdf.png",
    )
    parser.add_argument(
        "--output-summary-csv",
        type=Path,
        default=repo_root / "outputs" / "tables" / "no_survey_population_coverage_summary.csv",
    )
    parser.add_argument(
        "--output-cdf-csv",
        type=Path,
        default=repo_root / "outputs" / "tables" / "no_survey_population_coverage_cdf_points.csv",
    )
    ...
    service_styles = {
        ...
        "Sewage system access": {"color": "#c65a39"},
    }
    annotation_offsets = {
        ("Piped water", 10.0): (6, 8),
        ("Piped water", 15.0): (6, -16),
        ("Sewage system access", 10.0): (6, -16),
        ("Sewage system access", 15.0): (6, 8),
    }
